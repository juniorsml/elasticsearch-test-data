...
def reset_index(idx_name,idx_type):
    try:
        body = {"query":{"match_all": {}}}
        url = "%s/%s/%s/_delete_by_query" % (tornado.options.options.es_url, idx_name,idx_type)
        logging.info(url)
        logging.info(json.dumps(body))
        request = tornado.httpclient.HTTPRequest(url, headers=headers, method="POST", body=json.dumps(body) , request_timeout=240, auth_username=tornado.options.options.username, auth_password=tornado.options.options.password, validate_cert=tornado.options.options.validate_cert)
        response = tornado.httpclient.HTTPClient().fetch(request)
        logging.info('Reseting index  "%s" done   %s' % (idx_name, response.body))
    except tornado.httpclient.HTTPError:
        pass        
...
def get_data_for_format(format):
    ...
    if field_type == "bool":
        ...
        return_val = " ".join(words)
    elif field_type == "hardcoded":
        return_val = split_f[2] 
        ...
        if latitude != 0.0:
            lat , lon = generate_random_position(latitude , longitude)          
        else:
            lat , lon = generate_random_position(split_f[2] , split_f[3]) 
        
        latitude = lat
        longitude = lon        
        return_val = {'lat': lat, 'lon': lon}  
...
def generate_random_position(lat, lon):
   
    hex1 = '%012x' % random.randrange(16**12)                
    flt = float(random.randint(0,100))
    dec_lat = random.random()/100
    dec_lon = random.random()/100
    return  float(lat)+dec_lat,float(lon)+dec_lon 
...
def generate_test_data():
    ...
    if tornado.options.options.reset_index:
        reset_index(tornado.options.options.index_name,tornado.options.options.index_type)
...
if __name__ == '__main__':
    ...
    tornado.options.define("reset_index", type=bool, default=False, help="Force Reset the Elasticsearch index")
